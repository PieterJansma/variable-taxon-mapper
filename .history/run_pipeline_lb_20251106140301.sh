...
CTX=${CTX:-26000}                   # llama.cpp -c
SLOTS=${SLOTS:-1}                    # llama.cpp -np

LOG_DIR="${LOG_DIR:-$WORKDIR/logs}"
LLAMA_BIN="${LLAMA_BIN:-$WORKDIR/Repositories/llama.cpp/build/bin/llama-server}"
MODEL="${MODEL:-$WORKDIR/Models/GGUF/Qwen2.5-32B-Instruct/Qwen2.5-32B-Instruct-Q4_K_M.gguf}"

VTM_DIR="/groups/umcg-gcc/tmp02/users/umcg-pjansma/Repositories/variable-taxon-mapper" \
VTM_CFG="${VTM_CFG:-$VTM_DIR/config.example.toml}"
